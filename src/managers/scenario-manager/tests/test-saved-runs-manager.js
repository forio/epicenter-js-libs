...
var baseURL = (new URLService({ accountPath: runOptions.account, projectPath: runOptions.project })).getAPIPath('run');
describe('Saved Runs Manager', function () {
    ...
    beforeEach(function () {
        ...
        saveStub = sinon.stub(rs, 'save').callsFake(function (params) {
    ...
    describe('#getRuns', function () {
        ...
        beforeEach(function () {
            rs = new RunService(runOptions);
            queryStub = sinon.stub(rs, 'query').callsFake(function (options) {
        ...
        it('should query with model when provided', function () {
            ...
            var queryStub = sinon.stub(rs, 'query').callsFake(function (options) {
                return $.Deferred().resolve([
                    { id: 'run1' },
                    { id: 'run2' },
                ]).promise();
