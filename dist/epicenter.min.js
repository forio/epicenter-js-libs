...
(function (global){
var F={util:{},factory:{},transport:{},store:{},service:{},manager:{strategy:{}}};F.load=require("./env-load"),F.load(),F.util.query=require("./util/query-util"),F.util.run=require("./util/run-util"),F.util.classFrom=require("./util/inherit"),F.factory.Transport=require("./transport/http-transport-factory"),F.transport.Ajax=require("./transport/ajax-http-transport"),F.service.URL=require("./service/url-config-service"),F.service.Config=require("./service/configuration-service"),F.service.Run=require("./service/run-api-service"),F.service.File=require("./service/admin-file-service"),F.service.Variables=require("./service/variables-api-service"),F.service.Data=require("./service/data-api-service"),F.service.Auth=require("./service/auth-api-service"),F.service.World=require("./service/world-api-adapter"),F.service.State=require("./service/state-api-adapter"),F.service.User=require("./service/user-api-adapter"),F.service.Member=require("./service/member-api-adapter"),F.service.Asset=require("./service/asset-api-adapter"),F.service.Group=require("./service/group-api-service"),F.service.Introspect=require("./service/introspection-api-service"),F.store.Cookie=require("./store/cookie-store"),F.factory.Store=require("./store/store-factory"),F.manager.ScenarioManager=require("./managers/scenario-manager"),F.manager.RunManager=require("./managers/run-manager"),F.manager.AuthManager=require("./managers/auth-manager"),F.manager.WorldManager=require("./managers/world-manager"),F.manager.strategy["always-new"]=require("./managers/run-strategies/always-new-strategy"),F.manager.strategy["conditional-creation"]=require("./managers/run-strategies/conditional-creation-strategy"),F.manager.strategy.identity=require("./managers/run-strategies/identity-strategy"),F.manager.strategy["new-if-missing"]=require("./managers/run-strategies/new-if-missing-strategy"),F.manager.strategy["new-if-missing"]=require("./managers/run-strategies/new-if-missing-strategy"),F.manager.strategy["new-if-persisted"]=require("./managers/run-strategies/new-if-persisted-strategy"),F.manager.strategy["new-if-initialized"]=require("./managers/run-strategies/new-if-initialized-strategy"),F.manager.ChannelManager=require("./managers/epicenter-channel-manager"),F.service.Channel=require("./service/channel-service"),F.version="2.0.0",F.api=require("./api-version.json"),global.F=F,module.exports=F;
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
...
"use strict";function setRunInSession(sessionKey,run,sessionManager){sessionManager.getStore().set(sessionKey,JSON.stringify({runId:run.id}))}var Base=require("./identity-strategy");var SessionManager=require("../../store/session-manager");var classFrom=require("../../util/inherit");var AuthManager=require("../auth-manager");var keyNames=require("../key-names");var defaults={sessionKey:keyNames.STRATEGY_SESSION_KEY,path:""};var Strategy=classFrom(Base,{constructor:function(runService,condition,options){if(null===condition)throw new Error("Conditional strategy needs a condition to create a run");this._auth=new AuthManager,this.run=runService,this.condition="function"!=typeof condition?function(){return condition}:condition,this.options=$.extend(!0,{},defaults,options),this.sessionManager=new SessionManager(options),this.runOptions=this.options.run},runOptionsWithScope:function(){var userSession=this._auth.getCurrentUserSessionInfo();return $.extend({scope:{group:userSession.groupName}},this.runOptions)},reset:function(runServiceOptions){var me=this;var opt=this.runOptionsWithScope();return this.run.create(opt,runServiceOptions).then(function(run){return setRunInSession(me.options.sessionKey,run,me.sessionManager),run.freshlyCreated=!0,run})},getRun:function(){var sessionStore=this.sessionManager.getStore();var runSession=JSON.parse(sessionStore.get(this.options.sessionKey));var me=this;return runSession&&runSession.runId?this._loadAndCheck(runSession).fail(function(){return me.reset()}):this.reset()},_loadAndCheck:function(runSession){var shouldCreate=!1;var me=this;return this.run.load(runSession.runId,null,{success:function(run,msg,headers){shouldCreate=me.condition(run,headers)}}).then(function(run){if(shouldCreate){var opt=me.runOptionsWithScope();return me.run.create(opt).then(function(run){return setRunInSession(me.options.sessionKey,run,me.sessionManager),run.freshlyCreated=!0,run})}return run})}});module.exports=Strategy;
...
"use strict";var epiVersion=require("../api-version.json");var defaults={host:window.location.host,pathname:window.location.pathname};var UrlConfigService=function(config){function isLocalhost(){var isLocal=!options.host||"127.0.0.1"===options.host||0===options.host.indexOf("local.")||0===options.host.indexOf("localhost");return isLocal}var options=$.extend({},defaults,config);var actingHost=options.host;!actingHost&&isLocalhost()&&(actingHost="forio.com");var API_PROTOCOL="https";var HOST_API_MAPPING={"forio.com":"api.forio.com","foriodev.com":"api.epicenter.foriodev.com"};var publicExports={protocol:API_PROTOCOL,api:"",host:function(){var apiHost=HOST_API_MAPPING[actingHost]?HOST_API_MAPPING[actingHost]:actingHost;return apiHost}(),isCustomDomain:function(){var path=options.pathname.split("/");var pathHasApp=path&&"app"===path[1];return!isLocalhost()&&!pathHasApp}(),appPath:function(){var path=options.pathname.split("/");return path&&path[1]||""}(),accountPath:function(){var accnt="";var path=options.pathname.split("/");return path&&"app"===path[1]&&(accnt=path[2]),accnt}(),projectPath:function(){var prj="";var path=options.pathname.split("/");return path&&"app"===path[1]&&(prj=path[3]),prj}(),versionPath:function(){var version=epiVersion.version?epiVersion.version+"/":"";return version}(),isLocalhost:isLocalhost,getAPIPath:function(api){var PROJECT_APIS=["run","data","file"];var apiPath=this.protocol+"://"+this.host+"/"+this.versionPath+api+"/";return $.inArray(api,PROJECT_APIS)!==-1&&(apiPath+=this.accountPath+"/"+this.projectPath+"/"),apiPath}};var envConf=UrlConfigService.defaults;return $.extend(publicExports,envConf,config),publicExports};UrlConfigService.defaults={},module.exports=UrlConfigService;
