...
(function (global){
var F={_private:{},util:{},factory:{},transport:{},store:{},service:{},manager:{strategy:{}}};F.load=require("./env-load"),global.SKIP_ENV_LOAD||F.load(),F.util.query=require("./util/query-util"),F.util.run=require("./util/run-util"),F.util.classFrom=require("./util/inherit"),F._private.strategyutils=require("./managers/strategy-utils"),F.factory.Transport=require("./transport/http-transport-factory"),F.transport.Ajax=require("./transport/ajax-http-transport"),F.service.URL=require("./service/url-config-service"),F.service.Config=require("./service/configuration-service"),F.service.Run=require("./service/run-api-service"),F.service.File=require("./service/admin-file-service"),F.service.Variables=require("./service/variables-api-service"),F.service.Data=require("./service/data-api-service"),F.service.Auth=require("./service/auth-api-service"),F.service.World=require("./service/world-api-adapter"),F.service.State=require("./service/state-api-adapter"),F.service.User=require("./service/user-api-adapter"),F.service.Member=require("./service/member-api-adapter"),F.service.Asset=require("./service/asset-api-adapter"),F.service.Group=require("./service/group-api-service"),F.service.Introspect=require("./service/introspection-api-service"),F.service.Presence=require("./service/presence-api-service"),F.store.Cookie=require("./store/cookie-store"),F.factory.Store=require("./store/store-factory"),F.manager.ScenarioManager=require("./managers/scenario-manager"),F.manager.RunManager=require("./managers/run-manager"),F.manager.AuthManager=require("./managers/auth-manager"),F.manager.WorldManager=require("./managers/world-manager"),F.manager.SavedRunsManager=require("./managers/saved-runs-manager");var strategies=require("./managers/run-strategies");F.manager.strategy=strategies.list,F.manager.ChannelManager=require("./managers/epicenter-channel-manager"),F.service.Channel=require("./service/channel-service"),F.version="2.5.0",F.api=require("./api-version.json"),F.constants=require("./managers/key-names"),global.F=F,module.exports=F;
...
"use strict";function patchRunService(service,manager){if(service.patched)return service;var orig=service.do;return service.do=function(operation,params,options){var reservedOps=Object.keys(specialOperations);return reservedOps.indexOf(operation)===-1?orig.apply(service,arguments):specialOperations[operation].call(service,params,options,manager)},service.patched=!0,service}function sessionKeyFromOptions(options,runService){var config=runService.getCurrentConfig();var sessionKey=$.isFunction(options.sessionKey)?options.sessionKey(config):options.sessionKey;return sessionKey}function setRunInSession(sessionKey,run,sessionManager){sessionKey&&(delete run.variables,sessionManager.getStore().set(sessionKey,JSON.stringify(run)))}function RunManager(options){if(this.options=$.extend(!0,{},defaults,options),this.options.run instanceof RunService)this.run=this.options.run;else{if(util.isEmpty(this.options.run))throw new Error("No run options passed to RunManager");this.run=new RunService(this.options.run)}patchRunService(this.run,this),this.strategy=strategies.getBestStrategy(this.options),this.sessionManager=new SessionManager(this.options)}var strategies=require("./run-strategies");var specialOperations=require("./special-operations");var RunService=require("../service/run-api-service");var SessionManager=require("../store/session-manager");var util=require("../util/object-util");var keyNames=require("./key-names");var defaults={sessionKey:function(config){var baseKey=keyNames.STRATEGY_SESSION_KEY;var key=["account","project","model"].reduce(function(accum,key){return config[key]?accum+"-"+config[key]:accum},baseKey);return key}};RunManager.prototype={getRun:function(variables,options){var me=this;var sessionStore=this.sessionManager.getStore();var sessionContents=sessionStore.get(sessionKeyFromOptions(this.options,me.run));var runSession=JSON.parse(sessionContents||"{}");runSession.runId&&(runSession.id=runSession.runId);var authSession=this.sessionManager.getSession();return this.strategy.requiresAuth&&util.isEmpty(authSession)?(console.error("No user-session available",this.options.strategy,"requires authentication."),$.Deferred().reject("No user-session available").promise()):this.strategy.getRun(this.run,authSession,runSession,options).then(function(run){if(run&&run.id){me.run.updateConfig({filter:run.id});var sessionKey=sessionKeyFromOptions(me.options,me.run);if(setRunInSession(sessionKey,run,me.sessionManager),variables&&variables.length)return me.run.variables().query(variables).then(function(results){return run.variables=results,run}).catch(function(err){return run.variables={},console.error(err),run})}return run})},reset:function(options){var me=this;var authSession=this.sessionManager.getSession();return this.strategy.requiresAuth&&util.isEmpty(authSession)?(console.error("No user-session available",this.options.strategy,"requires authentication."),$.Deferred().reject("No user-session available").promise()):this.strategy.reset(this.run,authSession,options).then(function(run){if(run&&run.id){me.run.updateConfig({filter:run.id});var sessionKey=sessionKeyFromOptions(me.options,me.run);setRunInSession(sessionKey,run.id,me.sessionManager)}return run})}},RunManager.strategies=strategies,module.exports=RunManager;
...
var list={"conditional-creation":require("./conditional-creation-strategy"),"new-if-initialized":require("./deprecated/new-if-initialized-strategy"),"new-if-persisted":require("./deprecated/new-if-persisted-strategy"),none:require("./none-strategy"),multiplayer:require("./multiplayer-strategy"),"reuse-never":require("./reuse-never"),"reuse-per-session":require("./reuse-per-session"),"reuse-across-sessions":require("./reuse-across-sessions"),"reuse-last-initialized":require("./reuse-last-initialized")};list["always-new"]=list["reuse-never"],list["new-if-missing"]=list["reuse-per-session"],list["persistent-single-player"]=list["reuse-across-sessions"],module.exports={list:list,byName:function(strategyName){return list[strategyName]},getBestStrategy:function(options){var strategy=options.strategy;if(strategy||(strategy=options.strategyOptions&&options.strategyOptions.initOperation?"reuse-last-initialized":"reuse-per-session"),strategy.getRun)return strategy;var StrategyCtor="function"==typeof strategy?strategy:this.byName(strategy);if(!StrategyCtor)throw new Error("Specified run creation strategy was invalid:",strategy);var strategyInstance=new StrategyCtor(options);if(!strategyInstance.getRun||!strategyInstance.reset)throw new Error("All strategies should implement a `getRun` and `reset` interface",options.strategy);return strategyInstance.requiresAuth=StrategyCtor.requiresAuth,strategyInstance},register:function(name,strategy,options){strategy.options=options,list[name]=strategy}};
...
var ConfigService=require("./configuration-service");var TransportFactory=require("../transport/http-transport-factory");var SessionManager=require("../store/session-manager");var apiEndpoint="presence";module.exports=function(config){var defaults={account:void 0,project:void 0,groupName:void 0,transport:{}};this.sessionManager=new SessionManager;var serviceOptions=this.sessionManager.getMergedOptions(defaults,config);var urlConfig=new ConfigService(serviceOptions).get("server");serviceOptions.account&&(urlConfig.accountPath=serviceOptions.account),serviceOptions.project&&(urlConfig.projectPath=serviceOptions.project);var transportOptions=$.extend(!0,{},serviceOptions.transport,{url:urlConfig.getAPIPath(apiEndpoint)});serviceOptions.token&&(transportOptions.headers={Authorization:"Bearer "+serviceOptions.token});var http=new TransportFactory(transportOptions,serviceOptions);var getFinalParams=function(params){return"object"==typeof params?$.extend(!0,{},serviceOptions,params):serviceOptions};var publicAPI={markOnline:function(userId,options){options=options||{};var isString="string"==typeof userId;var objParams=getFinalParams(userId);if(!objParams.groupName&&!options.groupName)throw new Error("No groupName specified.");userId=isString?userId:objParams.userId;var groupName=options.groupName||objParams.groupName;var httpOptions=$.extend(!0,{},serviceOptions,options,{url:urlConfig.getAPIPath(apiEndpoint)+groupName+"/"+userId});return http.post({message:"online"},httpOptions)},markOffline:function(userId,options){options=options||{};var isString="string"==typeof userId;var objParams=getFinalParams(userId);if(!objParams.groupName&&!options.groupName)throw new Error("No groupName specified.");userId=isString?userId:objParams.userId;var groupName=options.groupName||objParams.groupName;var httpOptions=$.extend(!0,{},serviceOptions,options,{url:urlConfig.getAPIPath(apiEndpoint)+groupName+"/"+userId});return http.delete({},httpOptions)},getStatus:function(groupName,options){options=options||{};var objParams=getFinalParams(groupName);if(!groupName&&!objParams.groupName)throw new Error("No groupName specified.");groupName=groupName||objParams.groupName;var httpOptions=$.extend(!0,{},serviceOptions,options,{url:urlConfig.getAPIPath(apiEndpoint)+groupName});return http.get({},httpOptions)},getChannel:function(groupName,options){var ChannelManager=require("../managers/epicenter-channel-manager");options=options||{};var isString="string"==typeof groupName;var objParams=getFinalParams(groupName);if(!isString&&!objParams.groupName)throw new Error("No groupName specified.");groupName=isString?groupName:objParams.groupName;var cm=new ChannelManager(options);return cm.getPresenceChannel(groupName)}};$.extend(this,publicAPI)};
