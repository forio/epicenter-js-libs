...
"use strict";var Channel=require("../service/channel-service"),ChannelManager=function(options){if(!$.cometd)throw new Error("Cometd library not found. Please include epicenter-multiplayer-dependencies.js");if(!options||!options.url)throw new Error("Please provide an url for the cometd server");var cometd=new $.Cometd,defaults={url:"",logLevel:"info",websocketEnabled:!1,channel:{}},defaultCometOptions=$.extend(!0,{},defaults,options);cometd.websocketEnabled=defaultCometOptions.websocketEnabled,this.isConnected=!1,this.currentSubscriptions=[],this.options=defaultCometOptions;var connectionBroken=function(message){$(this).trigger("disconnect",message)},connectionSucceeded=function(message){$(this).trigger("connect",message)},me=this;cometd.configure(defaultCometOptions),cometd.addListener("/meta/connect",function(message){var wasConnected=this.isConnected;this.isConnected=message.successful===!0,!wasConnected&&this.isConnected?connectionSucceeded.call(this,message):wasConnected&&!this.isConnected&&connectionBroken.call(this,message)}.bind(this)),cometd.addListener("/meta/disconnect",connectionBroken),cometd.addListener("/meta/handshake",function(message){message.successful&&cometd.batch(function(){$(me.currentSubscriptions).each(function(index,subs){cometd.resubscribe(subs)})})}),cometd.addListener("/meta/subscribe",function(message){$(me).trigger("subscribe",message)}),cometd.addListener("/meta/unsubscribe",function(message){$(me).trigger("unsubscribe",message)}),cometd.addListener("/meta/publish",function(message){$(me).trigger("publish",message)}),cometd.addListener("/meta/unsuccessful",function(message){$(me).trigger("error",message)}),cometd.handshake(),this.cometd=cometd};ChannelManager.prototype.getChannel=function(options){options&&!$.isPlainObject(options)&&(options={base:options});var defaults={transport:this.cometd},channel=new Channel($.extend(!0,{},this.options.channel,defaults,options)),subs=channel.subscribe;channel.subscribe=function(){var subid=subs.apply(channel,arguments);return this.currentSubscriptions=this.currentSubscriptions.concat(subid),subid}.bind(this);var unsubs=channel.unsubscribe;return channel.unsubscribe=function(){for(var removed=unsubs.apply(channel,arguments),i=0;i<this.currentSubscriptions.length;i++)this.currentSubscriptions[i].id===removed.id&&this.currentSubscriptions.splice(i,1);return removed}.bind(this),channel},ChannelManager.prototype.on=function(){$(this).on.apply($(this),arguments)},ChannelManager.prototype.off=function(){$(this).off.apply($(this),arguments)},ChannelManager.prototype.trigger=function(){$(this).trigger.apply($(this),arguments)},module.exports=ChannelManager;
},{"../service/channel-service":24}],8:[function(require,module,exports){
"use strict";var ChannelManager=require("./channel-manager"),classFrom=require("../util/inherit"),urlService=require("../service/url-config-service"),AuthManager=require("./auth-manager"),session=new AuthManager,getFromSessionOrError=function(value,sessionKeyName){if(!value){var userInfo=session.getCurrentUserSessionInfo();if(!userInfo[sessionKeyName])throw new Error(sessionKeyName+" not found. Please log-in again, or specify "+sessionKeyName+" explicitly");value=userInfo[sessionKeyName]}return value},__super=ChannelManager.prototype,EpicenterChannelManager=classFrom(ChannelManager,{constructor:function(options){var userInfo=session.getCurrentUserSessionInfo(),defaults={server:{account:userInfo.account,project:userInfo.project}},defaultCometOptions=$.extend(!0,{},defaults,userInfo,options),urlOpts=urlService(defaultCometOptions.server);return defaultCometOptions.url||(defaultCometOptions.url=urlOpts.protocol+"://"+urlOpts.host+"/channel/subscribe"),this.options=defaultCometOptions,__super.constructor.call(this,defaultCometOptions)},getGroupChannel:function(groupName){groupName=getFromSessionOrError(groupName,"groupName");var baseTopic=["/group",this.options.server.account,this.options.server.project,groupName].join("/");return __super.getChannel.call(this,{base:baseTopic})},getWorldChannel:function(world,groupName){var worldid=$.isPlainObject(world)&&world.id?world.id:world;if(!worldid)throw new Error("Please specify a world id");groupName=getFromSessionOrError(groupName,"groupName");var baseTopic=["/game",this.options.server.account,this.options.server.project,groupName,worldid].join("/");return __super.getChannel.call(this,{base:baseTopic})},getUserChannel:function(world,user,groupName){var userid=$.isPlainObject(user)&&user.id?user.id:user,worldid=$.isPlainObject(world)&&world.id?world.id:world;if(!worldid)throw new Error("Please specify a world id");userid=getFromSessionOrError(userid,"userId"),groupName=getFromSessionOrError(groupName,"groupName");var baseTopic=["/users",this.options.server.account,this.options.server.project,groupName,worldid,userid].join("/");return __super.getChannel.call(this,{base:baseTopic})}});module.exports=EpicenterChannelManager;
...
"use strict";var classFrom=require("../../util/inherit"),Base={};module.exports=classFrom(Base,{constructor:function(runService){this.runService=runService},reset:function(){return $.Deferred().resolve().promise()},getRun:function(){return $.Deferred().resolve(this.runService).promise()}});
},{"../../util/inherit":37}],14:[function(require,module,exports){
"use strict";var classFrom=require("../../util/inherit"),IdentityStrategy=require("./identity-strategy"),WorldApiAdapter=require("../../service/world-api-adapter"),AuthManager=require("../auth-manager"),defaults={store:{synchronous:!0}},Strategy=classFrom(IdentityStrategy,{constructor:function(runService,options){this.runService=runService,this.options=$.extend(!0,{},defaults,options),this._auth=new AuthManager,this._loadRun=this._loadRun.bind(this),this.worldApi=new WorldApiAdapter(this.options.run)},reset:function(){var session=this._auth.getCurrentUserSessionInfo(),curUserId=session.userId,curGroupName=session.groupName;return this.worldApi.getCurrentWorldForUser(curUserId,curGroupName).then(function(world){return this.worldApi.newRunForWorld(world.id)}.bind(this))},getRun:function(){var session=this._auth.getCurrentUserSessionInfo(),curUserId=session.userId,curGroupName=session.groupName,worldApi=this.worldApi,_this=this,dtd=$.Deferred();if(!curUserId)return dtd.reject({statusCode:400,error:"We need an authenticated user to join a multiplayer world. (ERR: no userId in session)"},session).promise();var loadRunFromWorld=function(world){return world?worldApi.getCurrentRunId({filter:world.id}).then(_this._loadRun).then(dtd.resolve).fail(dtd.reject):dtd.reject({statusCode:404,error:"The user is not in any world."},{options:this.options,session:session})},serverError=function(error){dtd.reject(error,session,this.options)};return this.worldApi.getCurrentWorldForUser(curUserId,curGroupName).then(loadRunFromWorld).fail(serverError),dtd.promise()},_loadRun:function(id,options){return this.runService.load(id,null,options)}});module.exports=Strategy;
