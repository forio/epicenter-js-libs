...
"use strict";var RunService=require("../service/run-api-service");var SessionManager=require("../store/session-manager");var injectFiltersFromSession=require("./strategy-utils").injectFiltersFromSession;var SavedRunsManager=function(config){var defaults={scopeByGroup:!0,scopeByUser:!0};this.sessionManager=new SessionManager;var options=$.extend(!0,{},defaults,config);if(!options.run)throw new Error("No run options passed to SavedRunsManager");options.run instanceof RunService?this.runService=options.run:this.runService=new RunService(options.run),this.options=options};SavedRunsManager.prototype={save:function(run,otherFields){var param=$.extend(!0,{},otherFields,{saved:!0,trashed:!1});return this.mark(run,param)},remove:function(run,otherFields){var param=$.extend(!0,{},otherFields,{saved:!1,trashed:!0});return this.mark(run,param)},mark:function(run,toMark){var rs;var existingOptions=this.runService.getCurrentConfig();if(run instanceof RunService)rs=run;else{if(!run||"string"!=typeof run){if($.isArray(run)){var me=this;var proms=run.map(function(r){return me.mark(r,toMark)});return $.when.apply(null,proms)}throw new Error("Invalid run object provided")}rs=new RunService($.extend(!0,{},existingOptions,{id:run,autoRestore:!1}))}return rs.save(toMark)},getRuns:function(variables,filter,modifiers){var session=this.sessionManager.getSession(this.runService.getCurrentConfig());var scopedFilter=injectFiltersFromSession($.extend(!0,{},filter,{saved:!0,trashed:!1}),session,this.options);var opModifiers=$.extend(!0,{},{sort:"created",direction:"asc"},modifiers);var me=this;return this.runService.query(scopedFilter,opModifiers).then(function(savedRuns){if(!variables||!variables.length)return savedRuns;var promises=savedRuns.map(function(run){var config=$.extend(!0,{},me.runService.getCurrentConfig(),run,{autoRestore:!1});var rs=new RunService(config);var prom=rs.variables().query([].concat(variables)).then(function(variables){return run.variables=variables,run}).catch(function(err){return err&&console.error(err),run.variables={},run});return prom});return $.when.apply(null,promises).then(function(){return Array.apply(null,arguments)})})}},module.exports=SavedRunsManager;
},{"../service/run-api-service":37,"../store/session-manager":45,"./strategy-utils":26}],22:[function(require,module,exports){
"use strict";function ScenarioManager(config){var opts=$.extend(!0,{},defaults,config);config&&config.advanceOperation&&(opts.advanceOperation=config.advanceOperation),this.baseline=new RunManager({strategy:BaselineStrategy,sessionKey:"sm-baseline-run",run:strategyUtils.mergeRunOptions(opts.run,opts.currentRun),strategyOptions:{baselineName:opts.baselineRunName,initOperation:opts.advanceOperation}}),this.savedRuns=new SavedRunsManager($.extend(!0,{},opts.savedRuns,{run:opts.run}));var origGetRuns=this.savedRuns.getRuns;var me=this;this.savedRuns.getRuns=function(){var args=Array.apply(null,arguments);return me.baseline.getRun().then(function(){return origGetRuns.apply(me.savedRuns,args)})};var ignoreOperations=[].concat(opts.advanceOperation).map(function(opn){return Object.keys(opn)[0]});this.current=new RunManager({strategy:LastUnsavedStrategy,sessionKey:"sm-current-run",run:strategyUtils.mergeRunOptions(opts.run,opts.currentRun),strategyOptions:{ignoreOperations:ignoreOperations}}),this.current.saveAndAdvance=function(metadata){return me.current.getRun().then(function(){return me.current.run.serial(opts.advanceOperation)}).then(function(){return me.savedRuns.save(me.current.run,metadata)})}}var RunManager=require("./run-manager");var SavedRunsManager=require("./saved-runs-manager");var strategyUtils=require("./strategy-utils");var defaults={baselineRunName:"Baseline",advanceOperation:[{stepTo:"end"}],run:{},baselineRun:{},currentRun:{}};var BaselineStrategy=require("./scenario-strategies/baseline-strategy");var LastUnsavedStrategy=require("./scenario-strategies/reuse-last-unsaved");module.exports=ScenarioManager;
...
"use strict";var classFrom=require("../../util/inherit");var StateService=require("../../service/state-api-adapter");var injectFiltersFromSession=require("../strategy-utils").injectFiltersFromSession;var injectScopeFromSession=require("../strategy-utils").injectScopeFromSession;var Base={};module.exports=classFrom(Base,{constructor:function(options){var strategyOptions=options?options.strategyOptions:{};this.options=strategyOptions},reset:function(runService,userSession){var opt=injectScopeFromSession(runService.getCurrentConfig(),userSession);return runService.create(opt).then(function(createResponse){return runService.save({trashed:!1}).then(function(patchResponse){return $.extend(!0,{},createResponse,patchResponse)})})},getRun:function(runService,userSession){var filter=injectFiltersFromSession({trashed:!1},userSession);var me=this;var outputModifiers={sort:"created",direction:"desc"};return runService.filter(filter,outputModifiers).then(function(runs){if(!runs.length)return me.reset(runService,userSession);var lastRun=runs[0];if(lastRun.saved!==!0)return lastRun;var basedOnRunid=lastRun.id;var sa=new StateService;return sa.clone({runId:basedOnRunid,exclude:me.options.ignoreOperations}).then(function(response){return runService.load(response.run)}).then(function(run){return runService.save({trashed:!1}).then(function(patchResponse){return $.extend(!0,{},run,patchResponse)})})})}},{requiresAuth:!1});
