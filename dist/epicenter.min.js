...
(function (global){
var F={util:{},factory:{},transport:{},store:{},service:{},manager:{strategy:{}}};F.load=require("./env-load"),F.load(),F.util.query=require("./util/query-util"),F.util.makeSequence=require("./util/make-sequence"),F.util.run=require("./util/run-util"),F.util.classFrom=require("./util/inherit"),F.factory.Transport=require("./transport/http-transport-factory"),F.transport.Ajax=require("./transport/ajax-http-transport"),F.service.URL=require("./service/url-config-service"),F.service.Config=require("./service/configuration-service"),F.service.Run=require("./service/run-api-service"),F.service.File=require("./service/admin-file-service"),F.service.Variables=require("./service/variables-api-service"),F.service.Data=require("./service/data-api-service"),F.service.Auth=require("./service/auth-api-service"),F.service.World=require("./service/world-api-adapter"),F.service.State=require("./service/state-api-adapter"),F.service.User=require("./service/user-api-adapter"),F.service.Member=require("./service/member-api-adapter"),F.service.Asset=require("./service/asset-api-adapter"),F.store.Cookie=require("./store/cookie-store"),F.factory.Store=require("./store/store-factory"),F.manager.ScenarioManager=require("./managers/scenario-manager"),F.manager.RunManager=require("./managers/run-manager"),F.manager.AuthManager=require("./managers/auth-manager"),F.manager.WorldManager=require("./managers/world-manager"),F.manager.strategy["always-new"]=require("./managers/run-strategies/always-new-strategy"),F.manager.strategy["conditional-creation"]=require("./managers/run-strategies/conditional-creation-strategy"),F.manager.strategy.identity=require("./managers/run-strategies/identity-strategy"),F.manager.strategy["new-if-missing"]=require("./managers/run-strategies/new-if-missing-strategy"),F.manager.strategy["new-if-missing"]=require("./managers/run-strategies/new-if-missing-strategy"),F.manager.strategy["new-if-persisted"]=require("./managers/run-strategies/new-if-persisted-strategy"),F.manager.strategy["new-if-initialized"]=require("./managers/run-strategies/new-if-initialized-strategy"),F.manager.ChannelManager=require("./managers/epicenter-channel-manager"),F.service.Channel=require("./service/channel-service"),F.version="1.7.0",F.api=require("./api-version.json"),global.F=F,module.exports=F;
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
...
"use strict";var epiVersion=require("../api-version.json");var UrlConfigService=function(config){var API_PROTOCOL="https";var HOST_API_MAPPING={"forio.com":"api.forio.com","foriodev.com":"api.epicenter.foriodev.com"};var publicExports={protocol:API_PROTOCOL,api:"",host:function(){var host=window.location.host;return host&&-1===host.indexOf("local")||(host="forio.com"),HOST_API_MAPPING[host]?HOST_API_MAPPING[host]:"api."+host}(),appPath:function(){var path=window.location.pathname.split("/");return path&&path[1]||""}(),accountPath:function(){var accnt="";var path=window.location.pathname.split("/");return path&&"app"===path[1]&&(accnt=path[2]),accnt}(),projectPath:function(){var prj="";var path=window.location.pathname.split("/");return path&&"app"===path[1]&&(prj=path[3]),prj}(),versionPath:function(){var version=epiVersion.version?epiVersion.version+"/":"";return version}(),isLocalhost:function(){var host=window.location.host;return!host||-1!==host.indexOf("local")},getAPIPath:function(api){var PROJECT_APIS=["run","data","file"];var apiPath=this.protocol+"://"+this.host+"/"+this.versionPath+api+"/";return-1!==$.inArray(api,PROJECT_APIS)&&(apiPath+=this.accountPath+"/"+this.projectPath+"/"),apiPath}};var envConf={protocol:UrlConfigService.protocol,host:UrlConfigService.host};return $.extend(publicExports,envConf,config),publicExports};module.exports=UrlConfigService;
