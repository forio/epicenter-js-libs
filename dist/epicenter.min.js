...
"use strict";var Base=require("./none-strategy");var classFrom=require("../../util/inherit");var Strategy=classFrom(Base,{constructor:function(condition){if(null==condition)throw new Error("Conditional strategy needs a condition to create a run");this.condition="function"!=typeof condition?function(){return condition}:condition},reset:function(runService,userSession){var group=userSession&&userSession.groupName;var opt=$.extend({scope:{group:group}},runService.getCurrentConfig());return runService.create(opt).then(function(run){return run.freshlyCreated=!0,run})},getRun:function(runService,userSession,runIdInSession){var me=this;return runIdInSession?this.loadAndCheck(runService,userSession,runIdInSession).catch(function(){return me.reset(runService,userSession)}):this.reset(runService,userSession)},loadAndCheck:function(runService,userSession,runIdInSession){var shouldCreate=!1;var me=this;return runService.load(runIdInSession,null,{success:function(run,msg,headers){shouldCreate=me.condition(run,headers)}}).then(function(run){return shouldCreate?me.reset(runService,userSession):run})}});module.exports=Strategy;
},{"../../util/inherit":47,"./none-strategy":18}],13:[function(require,module,exports){
var list={"new-if-initialized":require("./new-if-initialized-strategy"),"new-if-persisted":require("./new-if-persisted-strategy"),"new-if-missing":require("./new-if-missing-strategy"),"always-new":require("./always-new-strategy"),multiplayer:require("./multiplayer-strategy"),"persistent-single-player":require("./persistent-single-player-strategy"),none:require("./none-strategy"),baseline:require("../scenario-strategies/baseline-strategy"),"new-if-stepped":require("../scenario-strategies/last-unsaved"),"conditional-creation":require("./conditional-creation-strategy")};module.exports={list:list,get:function(strategyName){return list[strategyName]},register:function(name,strategy,options){strategy.options=options,list[name]=strategy}};
...
"use strict";var classFrom=require("../../util/inherit");var IdentityStrategy=require("./none-strategy");var StateApi=require("../../service/state-api-adapter");var defaults={};var Strategy=classFrom(IdentityStrategy,{constructor:function(options){this.options=$.extend(!0,{},defaults,options),this.stateApi=new StateApi},reset:function(runService,userSession){var group=userSession.groupName;var opt=$.extend({scope:{group:group}},runService.getCurrentConfig());return runService.create(opt).then(function(run){return run.freshlyCreated=!0,run})},getRun:function(runService,userSession){var me=this;var params={"scope.group":userSession.groupName};return userSession.userId&&(params["user.id"]=userSession.userId),runService.query(params).then(function(runs){return me._loadAndCheck(runService,userSession,runs)})},_loadAndCheck:function(runService,userSession,runs){if(!runs||!runs.length)return this.reset(runService,userSession);var dateComp=function(a,b){return new Date(b.date)-new Date(a.date)};var latestRun=runs.sort(dateComp)[0];var me=this;var shouldReplay=!1;return runService.load(latestRun.id,null,{success:function(run,msg,headers){shouldReplay="persistent"===headers.getResponseHeader("pragma")}}).then(function(run){return shouldReplay?me.stateApi.replay({runId:run.id}).then(function(resp){return runService.load(resp)}):run})}},{requiresAuth:!0});module.exports=Strategy;
...
"use strict";function ScenarioManager(config){var sessionManager=new SessionManager;var serviceOptions=sessionManager.getMergedOptions(defaults,config);this.baseline=new RunManager({strategy:"baseline",sessionKey:"sm-baseline-run",run:serviceOptions}),this.current=new RunManager({strategy:"new-if-stepped",sessionKey:"sm-current-run",run:serviceOptions});var baseLineProm=this.baseline.getRun();this.savedRuns=new SavedRunsManager({run:serviceOptions});var orig=this.savedRuns.getRuns;this.savedRuns.getRuns=function(){var args=Array.apply(null,arguments);var me=this;return baseLineProm.then(function(){return orig.apply(me.savedRuns,args)})}}var RunManager=require("./run-manager");var SessionManager=require("../store/session-manager");var SavedRunsManager=require("./saved-run-manager");var defaults={baselineRunName:"Baseline"};module.exports=ScenarioManager;
