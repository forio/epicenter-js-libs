...
"use strict";var RunService=require("../service/run-api-service");var SessionManager=require("../store/session-manager");var SavedRunsManager=function(config){var defaults={scopeByGroup:!0,scopeByUser:!0};var options=$.extend(!0,{},defaults,config);if(!options.run)throw new Error("No run options passed to SavedRunsManager");options.run instanceof RunService?this.runService=options.run:this.runService=new RunService(options.run),this.options=options};SavedRunsManager.prototype={mark:function(run,toMark){var rs;if(run instanceof RunService)rs=run;else{if("string"==typeof Run)throw new Error("Invalid run object provided");var existingOptions=this.runService.getCurrentConfig();rs=new RunService($.extend(!0,{},existingOptions,{id:run}))}return rs.save(toMark)},save:function(run,otherFields){var param=$.extend(!0,{},otherFields,{saved:!0,trashed:!1});return this.mark(run,param)},remove:function(run,otherFields){var param=$.extend(!0,{},otherFields,{saved:!1,trashed:!0});return this.mark(run,param)},getRuns:function(variables,filter,options){var defaults={saved:!0,trashed:!1};var sm=new SessionManager;var session=sm.getSession(this.runService.getCurrentConfig());var isEmptySession=!!Object.keys(session).length;if(isEmptySession&&(this.scopeByGroup||this.scopeByUser))return $.Deferred().reject("SavedRunManager is missing session information").promise();this.options.scopeByGroup&&(defaults["scope.group"]=session.groupName),this.options.scopeByUser&&(defaults["user.id"]=session.userId);var actingFilter=$.extend(!0,{},defaults,filter);var opModifiers={sort:"created",direction:"asc"};return this.runService.query(actingFilter,opModifiers).then(function(savedRuns){if(!variables||!variables.length)return savedRuns;var promises=savedRuns.map(function(run){var rs=new RunService(run);var prom=rs.variables().query([].concat(variables)).then(function(variables){return run.variables=variables,run}).catch(function(err){return err&&console.error(err),run.variables={},run});return prom});return $.when.apply(null,promises).then(function(){return Array.apply(null,arguments)})})}},module.exports=SavedRunsManager;
},{"../service/run-api-service":36,"../store/session-manager":44}],22:[function(require,module,exports){
"use strict";function ScenarioManager(config){var serviceOptions=$.extend(!0,{},defaults,config);this.baseline=new RunManager({strategy:"baseline",sessionKey:"sm-baseline-run",run:serviceOptions.run}),this.current=new RunManager({strategy:"new-if-stepped",sessionKey:"sm-current-run",run:serviceOptions.run}),this.savedRuns=new SavedRunsManager({run:serviceOptions.run});var origGetRuns=this.savedRuns.getRuns;var me=this;this.savedRuns.getRuns=function(){var args=Array.apply(null,arguments);return me.baseline.getRun().then(function(){return origGetRuns.apply(me.savedRuns,args)})},this.current.save=function(metadata,operations){return me.current.getRun().then(function(){return me.current.run.do({stepTo:"end"})}).then(function(){return me.savedRuns.save(me.current.run,metadata)})}}var RunManager=require("./run-manager");var SavedRunsManager=require("./saved-run-manager");var defaults={baselineRunName:"Baseline"};module.exports=ScenarioManager;
